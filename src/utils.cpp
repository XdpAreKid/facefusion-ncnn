...
float GetIoU(const Bbox box1, const Bbox box2) {
  ...
  if (over_area == 0)
    return 0.0;
  float union_area = (box1.xmax - box1.xmin) * (box1.ymax - box1.ymin) +
...
transformLandMark(const std::vector<Landmarks> &lands) {
  std::vector<cv::Point2f> res;
  for (auto &land : lands) {
    res.push_back(cv::Point2f(land.x, land.y));
  }
  return res;
}

vector<int> nms(vector<Bbox> boxes, vector<float> confidences,
                const float nms_thresh) {
  sort(confidences.begin(), confidences.end(),
       [&confidences](size_t index_1, size_t index_2) {
...
Mat warp_face_by_face_landmark_5(const Mat temp_vision_frame, Mat &crop_img,
                                 const vector<Point2f> face_landmark_5,
                                 const vector<Point2f> normed_template,
                                 const Size crop_size) {
  vector<uchar> inliers(face_landmark_5.size(), 0);
  Mat affine_matrix = cv::estimateAffinePartial2D(
      face_landmark_5, normed_template, cv::noArray(), cv::RANSAC, 100.0);
  warpAffine(temp_vision_frame, crop_img, affine_matrix, crop_size,
             cv::INTER_AREA, cv::BORDER_REPLICATE);
  return affine_matrix;
}

Mat create_static_box_mask(const int *crop_size, const float face_mask_blur,
...
Mat paste_back(Mat temp_vision_frame, Mat crop_vision_frame, Mat crop_mask,
               Mat affine_matrix) {
  Mat inverse_matrix;
  ...
  warpAffine(crop_vision_frame, inverse_vision_frame, inverse_matrix, temp_size,
  ...
  for (int c = 0; c < 3; c++) {
    inverse_vision_frame_bgrs[c].convertTo(
        inverse_vision_frame_bgrs[c],
        CV_32FC1); ////注意数据类型转换，不然在下面的矩阵点乘运算时会报错的
    temp_vision_frame_bgrs[c].convertTo(
        temp_vision_frame_bgrs[c],
        CV_32FC1); ////注意数据类型转换，不然在下面的矩阵点乘运算时会报错的
  }
...
Mat blend_frame(Mat temp_vision_frame, Mat paste_vision_frame,
                const int FACE_ENHANCER_BLEND) {
  const float face_enhancer_blend = 1 - ((float)FACE_ENHANCER_BLEND / 100.f);
  Mat dstimg;
  cv::addWeighted(temp_vision_frame, face_enhancer_blend, paste_vision_frame,
                  1 - face_enhancer_blend, 0, dstimg);
  return dstimg;
}