#include "generator.h"
Generator::~Generator() { generator_net_.clear(); }
...
void Generator::PreProcess(const void *input_data,
                           std::vector<Tensor_t> &input_tensor) {}
void Generator::Normlize(const ncnn::Mat &output,
                         std::vector<float> &output_norm) {
  int size = output.c * output.h * output.w;

  output_norm.resize(size);
  std::copy((float *)output.data, ((float *)output.data) + size,
            output_norm.begin());

  for (int i = 0; i != output_norm.size(); ++i) {
    float val = output_norm[i] > 1.0
                    ? 1.0
                    : (output_norm[i] < -1.0 ? -1.0 : output_norm[i]);
    output_norm[i] = (val + 1.0) / 2.0;
  }
}

void Generator::Tensor2Image(std::vector<float> &output_tensor, int img_h,
                             int img_w, cv::Mat &output_img) {
  std::vector<cv::Mat> mat_list;
  for (int i = 0; i != 3; ++i) {
    cv::Mat mat = cv::Mat(img_h, img_w, CV_32FC1,
                          (void *)(output_tensor.data() + i * img_w * img_h));
...
void Generator::PostProcess(const std::vector<Tensor_t> &input_tensor,
...
void Generator::Run(const std::vector<Tensor_t> &input_tensor,
                    std::vector<Tensor_t> &output_tensor) {
...
int Generator::Process(const cv::Mat &input_img, void *result) {
  ...
  PostProcess(((CodeFormerResult_t *)result)->output_tensors, output_tensor,
              result);
