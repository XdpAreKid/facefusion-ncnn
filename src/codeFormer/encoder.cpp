#include "encoder.h"
Encoder::~Encoder() { encoder_net_.clear(); }

void Encoder::Topk(const ncnn::Mat &cls_scores, int topk,
  ...
  for (int n = 0; n != cls_scores.h; ++n) {
    ...
    std::partial_sort(vec.begin(), vec.begin() + 1, vec.end(),
                      [](const std::pair<float, int> &lhs,
                         const std::pair<float, int> &rhs) -> bool {
...
void Encoder::GetCodebookFeat(const ncnn::Mat &soft_one_hot,
...
void Encoder::Run(const std::vector<Tensor_t> &input_tensor,
...
void Encoder::PreProcess(const void *input_data,
                         std::vector<Tensor_t> &input_tensor) {
  ncnn::Mat in = ncnn::Mat::from_pixels(
      ((cv::Mat *)input_data)->data, ncnn::Mat::PIXEL_BGR2RGB,
      ((cv::Mat *)input_data)->cols, ((cv::Mat *)input_data)->rows);
  in.substract_mean_normalize(mean_vals_, norm_vals_);

  input_tensor.push_back(Tensor_t(in));
}
void Encoder::PostProcess(const std::vector<Tensor_t> &input_tensor,
  ...
  std::copy(output_tensor.begin(), output_tensor.end(),
            ((CodeFormerResult_t *)result)->output_tensors.begin());
}
